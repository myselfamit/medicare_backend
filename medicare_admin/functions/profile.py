...
logger = logging.getLogger("backend_admin_profile")
...
def get_admin_profile_data(request):
    """
    Get admin profile data from JSON file
    """
    try:
            ...
            return Response(
                {
                    ...
                    "error": "Access denied. Only administrators can access this endpoint.",
                    "timestamp": datetime.now().isoformat()
                },
        ...
        # Check if this is actually an admin profile
        if profile.get('user_type') != 'admin':
            return Response(
                {
                    "success": False,
                    "status_code": status.HTTP_403_FORBIDDEN,
                    "error": "Profile exists but is not an administrator profile",
                    "timestamp": datetime.now().isoformat()
                },
                status=status.HTTP_403_FORBIDDEN
            )
        ...
        logger.error(f"Error getting admin profile data: {e}")
        return Response(
            {
                "success": False,
...
def update_admin_profile_data(request):
    """
    Update admin profile data in JSON file
    """
    try:
        user_type = request.data.get("user_type", "").lower()
        email_id = request.data.get("email_id", "").lower()
            ...
            return Response(
                {
                    "success": False,
                    "status_code": status.HTTP_403_FORBIDDEN,
                    "error": "Access denied. Only administrators can update admin profiles.",
                    "timestamp": datetime.now().isoformat()
                },
                status=status.HTTP_403_FORBIDDEN
            )
        ...
        # Check if this is actually an admin profile
        if existing_profile.get('user_type') != 'admin':
            return Response(
                {
                    "success": False,
                    "status_code": status.HTTP_403_FORBIDDEN,
        ...
        updated_profile['user_type'] = 'admin'  # Ensure user type is preserved
        ...
        logger.error(f"Error updating admin profile data: {e}")
        return Response(
            {
                "success": False,
                "status_code": status.HTTP_500_INTERNAL_SERVER_ERROR,
...
def delete_admin_profile_data(request):
    """
    Delete admin profile data from JSON file
    """
    try:
        user_type = request.data.get("user_type", "").lower()
        email_id = request.data.get("email_id", "").lower()
            ...
            return Response(
                {
                    "success": False,
                    "status_code": status.HTTP_403_FORBIDDEN,
                    "error": "Access denied. Only administrators can delete admin profiles.",
                    "timestamp": datetime.now().isoformat()
                },
                status=status.HTTP_403_FORBIDDEN
            )
        ...
        # Check if this is actually an admin profile
        if profiles_data[email_id].get('user_type') != 'admin':
            return Response(
                {
                    "success": False,
                    "status_code": status.HTTP_403_FORBIDDEN,
        ...
        logger.error(f"Error deleting admin profile data: {e}")
        return Response(
            {
                "success": False,
                "status_code": status.HTTP_500_INTERNAL_SERVER_ERROR,
