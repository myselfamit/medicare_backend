...
from contacts.functions.signin import signin_function
    ...
    def post(self, request):
        ...
        try:
            if request.version == "v1":
                ...
                output = signin_function(request)
                return output
            else:
                raise ce.VersionNotSupported

        except ce.InvalidSlug as ins:
            logger.error("USER SIGN IN API VIEW : POST - {}".format(ins))
            raise

        except ce.ValidationFailed as vf:
            logger.error("USER SIGN IN API VIEW : POST - {}".format(vf))
            raise

        except ce.VersionNotSupported as vns:
            logger.error("USER SIGN IN API VIEW : POST - {}".format(vns))
            raise

        except Exception as e:
            logger.error("USER SIGN IN API VIEW : POST - {}".format(e))
